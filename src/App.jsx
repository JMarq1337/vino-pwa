...
const WineDetail=({wine,onEdit,onDelete,onMove,onAdjustConsumption})=>{
  ...
  return(
    <div style={desktopFloatingAside?{maxWidth:760,margin:"0 auto",position:"relative",paddingLeft:198}:{}}>
      {desktopFloatingAside&&(
        <div style={{position:"absolute",left:0,top:8,width:186,height:518,pointerEvents:"none",zIndex:3,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div style={{position:"absolute",left:"18%",right:"18%",bottom:18,height:42,background:"radial-gradient(ellipse at center, rgba(0,0,0,0.36) 0%, rgba(0,0,0,0.12) 44%, rgba(0,0,0,0) 100%)",filter:"blur(8px)"}}/>
          <WinePhotoImage src={wine.photo} alt={wine.name} style={{width:"100%",height:"100%",maxHeight:500,objectFit:"contain",objectPosition:"center",filter:"drop-shadow(0 20px 26px rgba(0,0,0,.34)) drop-shadow(0 3px 6px rgba(0,0,0,.18))",animation:"heroPhotoFloat .3s ease-out both"}}/>
        </div>
      )}
      <div style={desktopFloatingAside?{background:"var(--card)",border:"1px solid var(--border)",borderRadius:20,padding:14,boxShadow:"0 10px 28px var(--shadow)"}:{}}>
        {desktopFloatingAside?(
          ...
          <div style={{position:"relative",marginBottom:16}}>
            {hasPhoto&&(
              <div style={{position:"absolute",left:"50%",top:-26,transform:"translateX(-50%)",width:"min(92%,330px)",height:196,zIndex:3,pointerEvents:"none",display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
                <div style={{position:"absolute",left:"15%",right:"15%",bottom:8,height:36,background:`radial-gradient(ellipse at center, rgba(0,0,0,0.34) 0%, rgba(0,0,0,0.14) 46%, rgba(0,0,0,0) 100%)`,filter:"blur(8px)"}}/>
                <WinePhotoImage src={wine.photo} alt={wine.name} style={{width:"100%",height:"100%",objectFit:"contain",objectPosition:"center",filter:"drop-shadow(0 16px 22px rgba(0,0,0,.34))",animation:"heroPhotoFloat .34s ease-out both"}}/>
              </div>
            )}
            <div style={{borderRadius:16,background:`linear-gradient(140deg,${tc.dot} 0%,rgba(0,0,0,.24) 90%)`,padding:hasPhoto?"168px 20px 20px":"20px",position:"relative",overflow:"hidden",minHeight:hasPhoto?228:108,boxShadow:"inset 0 1px 0 rgba(255,255,255,.2)",animation:"heroGlassIn .25s ease-out"}}>
              <div style={{position:"absolute",right:-18,bottom:-18,opacity:0.12,pointerEvents:"none"}}><BrandLogo size={120} variant="mono"/></div>
              <div style={{position:"relative",zIndex:1}}>
                <WineTypePill type={type} label={varietal}/>
              </div>
...
const ProfileScreen=({wines,notes,theme,setTheme,profile,setProfile})=>{
  ...
  return(
    <div>
      ...
      <div style={{textAlign:"center",fontSize:12,color:"var(--sub)",fontFamily:"'Plus Jakarta Sans',sans-serif",opacity:0.6,marginBottom:8}}>Vinology v6.74 · {displayName}</div>
